...
def about(topic):
    ...
    before_index = paragraph.index(word) - 1
    after_index = paragraph.index(word) + len(word)
    if before_index >= 0 and after_index < len(paragraph):
        if paragraph[before_index] in alphabet or paragraph[after_index] in alphabet:
            return False
    """
